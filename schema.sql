...
DROP TABLE IF EXISTS task;
CREATE TABLE IF NOT EXISTS task (id INTEGER PRIMARY KEY, title TEXT, description TEXT, created_at DATE, updated_at DATE, start_date DATE, end_date DATE, project_id INTEGER, status_id INTEGER, tag_id INTEGER, assignee_id INTEGER, FOREIGN KEY(project_id) REFERENCES project(id) ON DELETE CASCADE, FOREIGN KEY(status_id) REFERENCES task_status(id) ON DELETE CASCADE, FOREIGN KEY(tag_id) REFERENCES tag(id) ON DELETE CASCADE, FOREIGN KEY(assignee_id) REFERENCES project_member(id) ON DELETE CASCADE);